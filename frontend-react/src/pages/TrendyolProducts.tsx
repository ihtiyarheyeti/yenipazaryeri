...
export default function TrendyolProducts(){
  ...
  async function load(){
    ...
    try {
      ...
      message.error("Trendyol ürünleri yüklenemedi");
    } finally {
      setLoading(false);
    }
  }
    ...
    {title:"İşlem", width:200, render:(_: any, r: any) => (
      <Space size="small">
        <Button 
          type="primary" 
          size="small" 
          onClick={()=>api(`/products/${r.id}/push/woo`,{method:'POST'})
            .then(()=>{message.success('WooCommerce\'a gönderildi'); load();})}
        >
          Eşle ve Gönder Woo
        </Button>
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16}}>
        ...
        <Button icon={<DownloadOutlined/>} onClick={()=>window.open(`${API_BASE}/csv/products/export?tenant_id=1&mp=1`)}>CSV Dışa Aktar</Button>
      </div>
