...
final class AuthController {
  public function login(array $p,array $b): array {
    ...
    $st=Database::pdo()->prepare("SELECT id,tenant_id,name,email,role,password,password_hash FROM users WHERE email=?");
    $st->execute([$email]);
    ...
    if(!is_string($hash) || $hash==='' || !password_verify($pass, $hash)){
      // Debug: Hash doğrulama
      error_log('VERIFY=' . (int)password_verify($pass, $hash));
      return ['ok'=>false,'error'=>'invalid credentials'];
    }

    // 2FA kontrolü - Geçici olarak devre dışı
    /*
    if((int)$u['twofa_enabled']===1){
      if(!$two){ $this->bumpLoginAttempt($email,$ip,false); return ['ok'=>false,'twofa_required'=>true,'error'=>'2fa_required']; }
      if(!Totp::verify($u['twofa_secret'],$two)){ $this->bumpLoginAttempt($email,$ip,false); return ['ok'=>false,'error'=>'invalid_2fa']; }
    }
    */

    // SUCCESS ⇒ sayaç sıfırla
    // $this->bumpLoginAttempt($email,$ip,true); // Geçici olarak devre dışı

    $token=Auth::generateToken((int)$u['id'],(int)($u['tenant_id'] ?? 1),$u['role'] ?? 'admin');
    $ref = \App\Controllers\SessionController::issueRefresh($u['id']);
    return ['ok'=>true,'token'=>$token,'refresh_token'=>$ref['token'],'refresh_expires_at'=>$ref['expires_at'],'user'=>[
      'id'=>$u['id'],'tenant_id'=>$u['tenant_id'] ?? 1,'name'=>$u['name'],'email'=>$u['email'],
      'role'=>$u['role'] ?? 'admin','twofa_enabled'=>false
    ]];
  }
  ...
  public function register(array $p,array $b): array {
    ...
    $check->execute([$email]);
    if($check->fetch()) return ['ok'=>false,'error'=>'email already exists'];
  ...
  // Yardımcı: Rate limiting için login attempt tracking
  ...
  public function refresh(array $p,array $b): array {
    ...
    // userId'yi refresh token tablosundan çek
    $st=\App\Database::pdo()->prepare("SELECT user_id FROM refresh_tokens WHERE token=?");
    $st->execute([$new['token']]); $uid=(int)$st->fetchColumn(); if(!$uid){ http_response_code(401); return ['ok'=>false,'error'=>'user_not_found']; }
    // kısa ömürlü erişim tokenını yeniden üret
    $jwt = \App\Auth::generateToken($uid, 1, 'admin'); // mevcut JWT üretim fonksiyonun
  ...
  public function logoutAll(array $p,array $b): array {
    $uid = $this->userId ?? 0; if($uid<=0){ http_response_code(401); return ['ok'=>false,'error'=>'unauthorized']; }
